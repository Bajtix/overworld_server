...
public class LuaVM
{
    ...
    public Table AddGlobalTable(string tableName)
    {
        ...
        {
            Logger.Log(Channel.Lua, Priority.Error, "Failed to add global Lua table {0}", tableName);
        }
    ...
    public bool SetGlobal(string key, object value)
    {
        ...
        {
            Logger.Log(Channel.Lua, Priority.Error, "Lua SetGlobal error: {0}", ex.DecoratedMessage);
        }
        return didSet;
    }
    ...
    public DynValue GetGlobal(string key)
    {
        ...
        {
            Logger.Log(Channel.Lua, Priority.Error, "Failed to get Lua global {0}", key);
        }
	...
	public DynValue GetGlobal(params object[] keys)
	{
		...
		{
			Logger.Log(Channel.Lua, Priority.Error, "Failed to get Lua global at '{0}'", 
	...
	public Table GetGlobalTable(string key)
	{
		...
		if (tableDyn != null)
		{
			...
			{
				Logger.Log(Channel.Lua, Priority.Error, "Lua global {0} is not type table, has type {1}", key, tableDyn.Type.ToString());
			}
		}
		return result;
	}
	...
	public Table GetGlobalTable(params object[] keys)
	{
		...
		if (tableDyn != null)
		{
			...
			{
				Logger.Log(Channel.Lua, Priority.Error, "Lua global {0} is not type table, has type {1}", keys, tableDyn.Type.ToString());
			}
		}
		return result;
	}
    ...
    public DynValue ExecuteString(string command)
    {
        ...
        {
            Logger.Log(Channel.Lua, Priority.Error, "Lua ExecuteString error: {0}", ex.DecoratedMessage);
        }
    ...
    public DynValue ExecuteScript(string filePath)
    {
        ...
        {
            Logger.Log(Channel.Lua, Priority.Error, "Lua ExecuteScript error: {0}", ex.DecoratedMessage);
        }
        catch (Exception ex)
        {
            Logger.Log(Channel.Lua, Priority.Error, "System ExecuteScript error: {0}", ex.Message);
        }
    ...
    public DynValue LoadScript(string filePath)
    {
        ...
        {
            Logger.Log(Channel.Lua, Priority.Error, "System ExecuteString error: {0}", ex.Message);
        }
    ...
    public DynValue Call(DynValue luaFunc, params object[] args)
    {
        ...
        if (luaFunc.IsNotNil() && luaFunc.Type == DataType.Function)
        {
            ...
            {
                Logger.Log(Channel.Lua, Priority.Error, "Lua Call error: {0}", ex.DecoratedMessage);
            }
        }
        else
        {
            Logger.Log(Channel.Lua, Priority.Error, "Invalid lua function passed to LuaVM::Call");
        }
    ...
    public DynValue Call(string functionName, params object[] args)
    {
        ...
        if (!string.IsNullOrEmpty(functionName))
        {
            ...
            if (func.Type == DataType.Function)
            {
                ...
                {
                    Logger.Log(Channel.Lua, Priority.Error, "Lua error calling function {0}: {1}", functionName, ex.DecoratedMessage);
                }
            }
            else
            {
                Logger.Log(Channel.Lua, Priority.Error, "Failed to find lua function '{0}'", functionName);
            }
        }
        return result;
    }
    ...
    /// <summary>
    /// [mod] A method for adding only a single API into the VM, added by bajtix
    /// </summary>
    /// <param name="tps">The API to add, must derive from LuaAPIBase</param>
    public void AttachCustomAPI(Type tps) 
    {
        if (tps == null) return;

        // Instantiate the type
        LuaAPIBase ist = Activator.CreateInstance(tps) as LuaAPIBase;

        // Update VM
        ist.AddAPIToLuaInstance(this);
    }
